...
export class AuthService {
  ...
  async register(createUserDTO: CreateUserDTO): Promise<void | IErrorResponse> {
    try {
      ...
      const codeSent = await axios.post(
        `${process.env.AUTH0_API}/passwordless/start`,
        {
      ...
      if (error.code === 11000) {
        const loginResponse: IErrorResponse = {
          status: 'failed',
  ...
  async remove(
    removeUserPayload: CheckUserDTO,
  ...
  async verifyEmailOTP(email: string, code: string): Promise<any> {
    try {
      const verified = await axios.post(
        `${process.env.AUTH0_API}/oauth/token`,
        {
